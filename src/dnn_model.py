...
# Load your preprocessed data (correct path)
df = pd.read_csv('data/preprocessed_twitch_reviews.csv')  # Correct path

# Ensure that NaN values in 'content' are filled with an empty string
df['content'] = df['content'].fillna('')  # Fill NaN with empty string
...
X = df['content'].apply(lambda x: len(x.split()))  # Feature: Length of cleaned content (as a placeholder feature)
...
torch.save(model.state_dict(), "sentiment_analysis_dnn_model.pth")
print("Model saved as 'sentiment_analysis_dnn_model.pth'")
